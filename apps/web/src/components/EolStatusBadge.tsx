...
export function EolStatusBadge({ eolDate, isEol }: EolStatusBadgeProps) {
  ...
  if (daysUntilEol <= 30) {
    return (
      <span className="px-2 py-1 text-xs font-semibold rounded bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200">
        30日以内
      </span>
    );
  }
  ...
  if (daysUntilEol <= 90) {
    return (
      <span className="px-2 py-1 text-xs font-semibold rounded bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200">
        90日以内
      </span>
    );
  }
