  ...
  try {
    ...
    for (const cycle of approachingCycles.results) {
      ...
      await dispatchNotification(env, 'eol_approaching', {
        ...
        severity: daysUntilEol <= 7 ? 'critical' : daysUntilEol <= 14 ? 'high' : 'medium',
      });
    }
    ...
    console.log(
      `[EOL Check] Found ${approachingCycles.results.length} approaching EOL, ${expiredCycles.results.length} expired`,
    );
  } catch (err) {
