...
export function parseCPE(cpe: string): ParsedCPE | null {
  ...
  if (cpe.startsWith('cpe:2.3:')) {
    ...
    const version = parts[5] && parts[5] !== '*' ? decodeURIComponent(parts[5].replace(/\\\:/g, ':')) : undefined;
  ...
  if (cpe.startsWith('cpe:/')) {
    ...
    const version = parts[3] && parts[3] !== '*' ? decodeURIComponent(parts[3].replace(/\\\:/g, ':')) : undefined;
