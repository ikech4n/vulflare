...
import { fetchVendorList, fetchProductList } from './myjvn-api.ts';
...
describe('fetchVendorList', () => {
  ...
  it('HTMLエンティティ（&amp; &lt; &gt; &quot; &apos;）をデコードする', async () => {
    fetchMock
      ...
      .reply(
        200,
        makeVendorListXml([{ vid: 'v001', vname: 'AT&amp;T &lt;Corp&gt;' }]),
      );
...
describe('fetchProductList', () => {
  ...
  it('製品が0件 → vendorNameは取得し、productsは空配列を返す', async () => {
    fetchMock
      ...
      .reply(
        200,
        '<Response><Vendor vid="v001" vname="Empty Vendor"></Vendor></Response>',
      );
