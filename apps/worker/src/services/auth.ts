...
export async function verifyJwt<T extends Record<string, unknown>>(
  ...
  try {
    ...
    const key = await importHmacKey(secret);
    const data = `${header}.${body}`;
    const rawSig = Uint8Array.from(atob(sig.replace(/-/g, "+").replace(/_/g, "/")), (c) =>
      c.charCodeAt(0),
    );
...
export function makeAccessToken(userId: string, role: string, secret: string): Promise<string> {
  const now = Math.floor(Date.now() / 1000);
  return signJwt({ sub: userId, role, type: "access", iat: now, exp: now + 15 * 60 }, secret);
}

export function makeRefreshToken(userId: string, jti: string, secret: string): Promise<string> {
  const now = Math.floor(Date.now() / 1000);
  return signJwt(
