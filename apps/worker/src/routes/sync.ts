...
import { getSyncSettings, updateSyncSettings, type SyncSettings } from '../services/sync-settings.ts';
import { handleJvnSync } from '../scheduled/jvn-sync.ts';
import { validate } from '../validation/middleware.ts';
import { updateSyncSettingsSchema, cancelSyncSchema } from '../validation/schemas.ts';
import { fetchVendorList, fetchProductList, type MyjvnVendor, type MyjvnProduct } from '../services/myjvn-api.ts';
...
syncRoutes.get('/filter-options', authMiddleware, async (c) => {
  try {
    ...
    const [vendorRows, productRows] = await Promise.all([
      c.env.DB.prepare('SELECT name FROM jvn_vendors ORDER BY name COLLATE NOCASE')
        .all<{ name: string }>(),
      c.env.DB.prepare('SELECT name FROM jvn_products ORDER BY name COLLATE NOCASE')
        .all<{ name: string }>(),
    ]);
    ...
    console.log(`Returning ${vendors.length} vendors and ${products.length} products from JVN master tables`);
    ...
    if (vendors.length === 0 && products.length === 0) {
      ...
      return c.json({
        vendors: [
          'Microsoft', 'Oracle', 'Adobe', 'Apache', 'Google', 'Apple',
          'Cisco', 'IBM', 'Red Hat', 'Mozilla', 'Linux', 'PHP',
          'WordPress', 'Jenkins', 'Atlassian', 'VMware', 'Samsung'
        ],
        products: [
          'Windows', 'Linux Kernel', 'Chrome', 'Firefox', 'Safari',
          'Java', 'PHP', 'MySQL', 'PostgreSQL', 'Apache HTTP Server',
          'nginx', 'WordPress', 'OpenSSL', 'jQuery', 'Node.js'
        ]
      });
    }
  ...
  try {
    ...
    const cacheResults = await c.env.DB.prepare(
      `SELECT vid, vname, fetched_at FROM jvn_vendor_cache
       ...
       LIMIT 1000`
    ).bind(`%${query}%`).all<{ vid: string; vname: string; fetched_at: string }>();

    // キャッシュが空または古い場合、MyJVN APIから取得
    const now = new Date();
    const shouldRefetch = cacheResults.results.length === 0 ||
      (cacheResults.results[0] &&
       (now.getTime() - new Date(cacheResults.results[0].fetched_at).getTime()) > CACHE_TTL_HOURS * 60 * 60 * 1000);
    ...
    if (shouldRefetch) {
      ...
      const refreshed = await c.env.DB.prepare(
        `SELECT vid, vname FROM jvn_vendor_cache
         ...
         ORDER BY vname COLLATE NOCASE
         LIMIT 1000`
      ).bind(`%${query}%`).all<{ vid: string; vname: string }>();
...
syncRoutes.get('/jvn-vendors/:vid/products', authMiddleware, async (c) => {
  ...
  try {
    ...
    ).bind(vendorId).all<{ pid: string; pname: string; cpe: string; fetched_at: string }>();
    ...
    if (shouldRefetch) {
      ...
      const refreshed = await c.env.DB.prepare(
        `SELECT pid, pname, cpe FROM jvn_product_cache
         WHERE vendor_vid = ?
         ORDER BY pname COLLATE NOCASE`
      ).bind(vendorId).all<{ pid: string; pname: string; cpe: string }>();
