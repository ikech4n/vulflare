...
authRoutes.get("/initialized", async (c) => {
  const result = await c.env.DB.prepare("SELECT COUNT(*) as cnt FROM users").first<{
    cnt: number;
  }>();
...
authRoutes.post("/register", rateLimitPresets.register, validate(registerSchema), async (c) => {
  ...
  const countResult = await c.env.DB.prepare("SELECT COUNT(*) as cnt FROM users").first<{
    cnt: number;
  }>();
  ...
  if (userCount > 0) {
    ...
    const payload = await verifyJwt<{ sub: string; role: string; type: string }>(
      token,
      c.env.JWT_SECRET,
    );
...
authRoutes.post("/login", rateLimitPresets.login, validate(loginSchema), async (c) => {
  ...
  await sessionTokenRepo.create(
    c.env.VULFLARE_KV_SESSIONS,
    jti,
    user.id,
    tokenHash,
