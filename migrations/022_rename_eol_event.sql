-- EOLイベント名を eol_detected から eol_approaching に変更
...
-- データを移行（eol_detectedをeol_approachingに変換）
INSERT INTO notification_rules (id, channel_id, event_type, filter_config, is_active, created_at, updated_at)
SELECT
  id,
  channel_id,
  CASE WHEN event_type = 'eol_detected' THEN 'eol_approaching' ELSE event_type END as event_type,
  filter_config,
  is_active,
  created_at,
  updated_at
